...
%the spiral path of tool 2--------------------TOOL 2
...
writerObj=VideoWriter('In-In feed with equal-angle path.avi');%make a movie
...
for i=1:size(xa1,2)%
    ...
    if (d_dt>710*i_t) || (i==size(xa1,2))
        ...
        title({['In-In feed with equal-angle path'];['Total dwell time=',num2str(d_dt1),'h']});
